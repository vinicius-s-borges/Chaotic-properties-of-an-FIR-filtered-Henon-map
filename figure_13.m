...
%Filter parameters
coef = 0:1:40; % coefficients
Ncoef = length(coef); % numbers coefficients
Nganhos = 100; % numbers of gains
ganhos = linspace(0.000001,2.5,Nganhos); % gains
...
for iterag=1:Nganhos,  
    ganho = ganhos(iterag);
    c = poly(-ones(coef(iteraNs),1));
    c = (poly(-ones(coef(iteraNs),1))/sum(c))*ganho;
    Ns=length(c);
    gamma = sum(c);
...
save('variables_lyapunov_teste_4')
%%
figure
...
%%
figure
pcolor(ncoef_int(1,3:82),ganhos,h_color_1_int(3:82,:)')
ylabel({'$G$'},'Interpreter','latex')
xlabel({'$N_{Z}$'},'Interpreter','latex')
shading flat
hold on
pcolor(ncoef_int(1,3:82),ganhos,h_color_2_int(3:82,:)')
pcolor(ncoef_int(1,3:82),ganhos,h_color_3_int(3:82,:)')
pcolor(ncoef_int(1,3:82),ganhos,h_color_4_int(3:82,:)')
shading flat
plot(41.5,0.5,'x') 
set(gca,'layer','top','FontSize',24)
plot(25.5,0.8,'x') 
...
grid on
ax = gca;
ax.XTick = [3.5 11.5 21.5 31.5 41.5 51.5 61.5 71.5 81.5];
