...
Nws = 500;
w=linspace(0.00001,3.14,Nws);
Nganhos = 500; 
ganhos = linspace(0.0001,3,Nganhos); 
%%
...
for iterag=1:Nganhos,  
    ganho = ganhos(iterag);
    ws = w(iteraws);
    c = poly([exp(1j*ws) -1 exp(-1j*ws)]);
    c = (poly([exp(1j*ws) -1 exp(-1j*ws)])/sum(c))*ganho;
    Ns=length(c);
    gamma = ganho;
...
save('variables_lyapunov_teste_2_czpi')
%%
